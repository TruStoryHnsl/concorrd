...
> **Phase 8/9 follow-up — mesh audio capture + playback landed**
> (`feat/mesh-media-audio-capture`): closes the
> `TODO(mesh-media-followup)` markers from the Phase 8 follow-up. Native
> uses **cpal** (Linux / macOS / Windows) for mic input + speaker output,
> **audiopus** for opus encode/decode @ 48kHz mono / 20ms frames, and the
> **rtp** crate for RTP packetization (PT 111, monotonic seq/ts). Browser
> uses **`getUserMedia({ audio: true })` + `RTCPeerConnection.addTrack`**
> on the send side and remote **`ontrack` → `HTMLAudioElement` playback**
> on the receive side. The two-peer native↔native mesh call now actually
> carries audio bytes; LiveKit remains the SFU fallback for >8 / web-only /
> iOS. iOS mesh is explicitly gated off via `#[cfg(not(target_os = "ios"))]`
> in `src-tauri/src/servitude/voice/audio.rs`; iOS callers receive
> `VoiceError::AudioNotSupported` from `WebRtcMediaPeer::start_audio_capture`
> and the path-selector falls back to LiveKit. The AVAudioEngine path for
> iOS is queued as a `TODO(mesh-media-followup-v2)` follow-up.
>
> What's deferred to a future PR: opus DTX/FEC, NACK, congestion control,
> jitter buffer, echo cancellation, sample-rate conversion on non-48kHz
> output devices. Quality tuning is its own scope.
>
...
> **Phase 9 follow-up (browser P2P UI surface) landed** (`feat/browser-p2p-ui-surface`): Settings → Profile in the docker/web build now exposes the browser's ephemeral session identity, swarm status, peer-card display + scanner, and a localStorage-backed peer store. The mesh voice path, browser-side peer pairing, and LAN-or-WAN peer-to-peer connections all work from a plain browser tab now. **Identity row** — `client/src/libp2p/fingerprint.ts` re-implements the native `fingerprint_for()` algorithm (base32-nopad of SHA-256-of-public-key, truncated to 16 chars) so the browser session row shows the same fingerprint format a native install would for the same pub-key bytes; labelled "Session identity (ephemeral)" with an info-tooltip explaining the per-tab reset semantics. **Swarm status** — the four-row native block (Our PeerId / Listening on / Peers connected / Last event) is mirrored on web, reading directly from `getBrowserNodeIfStarted()` plus live `peer:connect` / `peer:disconnect` listeners. **Peer card** — `PeerCardDisplay` derives the card on web from `getBrowserIdentity()` + the running browser node's multiaddrs; a "(session card)" subtitle warns users the card is ephemeral. **Scanner** — `PeerCardScanner` exposes the camera tab anywhere `navigator.mediaDevices.getUserMedia` is available (Tauri WebView + every modern browser), feature-detect not browser-sniff; paste path stays universal. **Peer store** — `client/src/stores/peerStoreBrowser.ts` adds a localStorage-backed peer store under `concord:browser:peer-store`, mirroring the native `KnownPeer` wire model so the rest of the UI doesn't branch; quota errors are routed into the store's `error` field rather than crashing the React tree. **Lifecycle** — `useBrowserLibp2p({ enabled: true })` moved from `PairedPeersSection` up to the `ProfileTab` body, so opening Settings → Profile is the swarm-boot trigger on web. **LAN subsection** — hidden entirely on web; browsers have no portable mDNS surface to enumerate from a tab. **Tests** — 1 case in `client/src/libp2p/__tests__/fingerprint.test.ts` (test vector against the all-zero public key), 4 cases in `client/src/stores/__tests__/peerStore.web.test.ts` (add/list/remove/reload), 3 cases in `client/src/components/settings/__tests__/ProfileTab.web.test.tsx` (browser-identity row, swarm peer count, enabled Add-a-peer button), 2 cases in `client/src/components/peers/__tests__/PeerCardDisplay.web.test.tsx` (browser QR card, session subtitle). Two existing `peerStore.test.ts` cases were updated from the legacy "native-only sentinel" expectations to the new localStorage-backed behavior. Baseline 484 → 496 passing.
