...
3. Concorrd.com owns the production bot, so other instances can't re-use it.
   Every new instance needs its own admin to create a Discord app, which is
   operational friction with no privacy benefit for non-admin users.
