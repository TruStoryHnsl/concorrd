...
interface ServerSidebarProps {
  mobile?: boolean;
  onServerSelect?: () => void;
  /** Source management callbacks — when provided, source tiles render at the bottom */
  onAddSource?: () => void;
  onExplore?: () => void;
  onSourceOpen?: (sourceId: string) => void;
}

export const ServerSidebar = memo(function ServerSidebar({ mobile, onServerSelect, onAddSource, onExplore, onSourceOpen }: ServerSidebarProps) {
  ...
  return (
    <div className="w-16 bg-surface" style={{ height: "100%" }}>
      ...
      {/* ── Bottom region: federated + sources ── */}
      <div className="overflow-y-auto overflow-x-hidden py-2 flex flex-col items-center justify-end gap-2 [&>*]:shrink-0 border-t border-outline-variant/20" style={{ height: "50%" }}>
          ...
          {sources.map((source) => {
            ...
            switch (source.platform) {
              ...
              case "discord-account":
                bg = "bg-[#5865F2]";
                icon = <span className="material-symbols-outlined text-white text-lg">person</span>;
                break;
              case "matrix":
                bg = "bg-teal-700";
            ...
            return (
              <div key={source.id} className="relative group flex-shrink-0">
                <button
                  onClick={() => toggleSource(source.id)}
                  onContextMenu={(e) => { e.preventDefault(); onSourceOpen(source.id); }}
                  title={`${label}${enabled ? "" : " (hidden)"} — right-click for settings`}
                  className={`btn-press w-8 h-8 flex items-center justify-center rounded-lg hover:rounded-xl transition-all hover:scale-105 ${bg} ${
      ...
      {onAddSource && (
        <button
          onClick={onAddSource}
          title="Add Source"
          className="btn-press w-8 h-8 rounded-lg hover:rounded-xl bg-white/5 hover:bg-white/10 flex items-center justify-center text-on-surface-variant/50 hover:text-on-surface-variant transition-all flex-shrink-0"
        >
