    ...
    fn handle_swarm_event(&mut self, event: SwarmEvent<ConcordBehaviourEvent>) {
        match event {
            ...
            SwarmEvent::OutgoingConnectionError { peer_id, error, .. } => {
                // Log at info during development so timeout patterns are visible.
                // TODO(production): downgrade to debug once connection logic is stable.
                info!(?peer_id, %error, "outgoing connection error (peer unreachable)");
            }

            SwarmEvent::IncomingConnectionError { error, .. } => {
                info!(%error, "incoming connection error");
            }
