...
impl DiscordBridgeTransport {
    ...
    ///
    /// The defaults written here are intentionally minimal placeholders
    /// — Wave 4 wires the real `BridgesTab` UI that lets the user paste
    /// a Discord bot token which is then persisted to Stronghold and
    /// folded into `config.yaml` on the next start.
    async fn ensure_config_and_registration(
        ...
        if !tokio::fs::try_exists(&config_path).await.unwrap_or(false) {
            // Placeholder config — Wave 4 will swap in a populated
            // form sourced from Stronghold. Minimal keys that let
            // mautrix-discord start without crashing are enough for
            // the Rust unit-test harness.
            let contents = format!(
                "# Generated by Concord (INS-024 Wave 3 placeholder)\n\
                 # Wave 4 wires BridgesTab.tsx to populate the real token.\n\
                 homeserver:\n\
                 ...
                 \x20\x20as_token: CONCORD_PLACEHOLDER_AS_TOKEN\n\
                 \x20\x20hs_token: CONCORD_PLACEHOLDER_HS_TOKEN\n\
                 bridge:\n\
            ...
            .unwrap_or(false)
        {
            let contents = format!(
                "# Generated by Concord (INS-024 Wave 3 placeholder)\n\
                 ...
                 as_token: CONCORD_PLACEHOLDER_AS_TOKEN\n\
                 hs_token: CONCORD_PLACEHOLDER_HS_TOKEN\n\
