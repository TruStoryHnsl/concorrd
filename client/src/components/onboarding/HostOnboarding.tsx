...
export function HostOnboarding({
  ...
  const handleBringUp = useCallback(async () => {
    ...
    // Persist the instance display name as the vanity instance name
    // — replaces "local" on the source-rail home tile and rides in
    // the libp2p Identify protocol so peers can confirm they reached
    // the right device. Best-effort: a failure here MUST NOT roll
    // back a successful owner-registration (the homeserver is up,
    // the user has an account, the wizard succeeded).
    if (serverName.displayName.trim()) {
      try {
        await useInstanceNameStore
          .getState()
          .set(serverName.displayName.trim());
      } catch (err) {
  ...
  if (step === "spinning") {
    const phaseText = humanizePhase(spinnerStatus.phase);
    const statusLine = spinnerStatus.detail
      ? `${phaseText} (${spinnerStatus.detail})`
      : phaseText;
    return (
      <div
