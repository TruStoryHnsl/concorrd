...
- **INS-026:** GitHub bug report integration — new implementation task introduced by Appendix B #3's resolution. Extends `submit_bug_report` in `server/routers/admin.py` to auto-create GitHub issues via `POST /repos/TruStoryHnsl/concord/issues`, authenticated by a server-side fine-grained PAT.
