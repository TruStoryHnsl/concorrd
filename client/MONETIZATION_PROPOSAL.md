...
3. ~~**Crash reporting**~~ → **RESOLVED 2026-04-08 — no external dependencies, GitHub-integrated bug reports**. User directive: "no external dependencies. Bug reports should be integrated into the github workflow." **No Sentry, no Datadog, no Bugsnag, no third-party crash SDK of any kind.** The client remains crash-tolerant via in-place React error boundaries (e.g. `ChartErrorBoundary` in `MessageContent.tsx:374` → `InvalidChartPill` fallback) and `console.warn` logging that lands in Tauri's native stdout stream for debug builds. The user-initiated bug report path (`BugReportModal.tsx` → `submitBugReport` → `POST /api/bug-reports`) is retained and **extended to auto-create a GitHub issue** in the concord repo when a report is filed. Implementation sketch: the server-side `submit_bug_report` handler in `server/routers/admin.py:127` stays the source of truth (persists the row to the `bug_reports` table for audit + admin list UI) but ALSO calls the GitHub REST API (`POST /repos/TruStoryHnsl/concord/issues`) with a body composed of the title, description, and a formatted `<details>` block containing the `system_info` JSON (userAgent, URL, voice state, etc.). Auth via a single server-side env var `GITHUB_BUG_REPORT_TOKEN` (fine-grained PAT scoped to `issues:write` on the concord repo only). If the GitHub call fails, the DB row is still written and the failure is logged — the user's report is NEVER lost to a GitHub outage. Admin UI gains a small "View on GitHub" link per report once the issue number is persisted on the row. **This is a new implementation task** tracked separately as `INS-028 GitHub bug report integration`. No app-store SDK changes. No client-side credential handling. The client just POSTs to Concord's own endpoint as it already does.
...
- **INS-028:** GitHub bug report integration — new implementation task introduced by Appendix B #3's resolution. Extends `submit_bug_report` in `server/routers/admin.py` to auto-create GitHub issues via `POST /repos/TruStoryHnsl/concord/issues`, authenticated by a server-side fine-grained PAT.
...
*Proposal fully resolved 2026-04-08 via user directive. All six Appendix B open questions are now settled — crash reporting was closed by the no-external-dependencies + GitHub-integration answer (captured as the new INS-028 implementation task). Ready for Phase 1 implementation when the INS-020 native mobile shell exists.*
